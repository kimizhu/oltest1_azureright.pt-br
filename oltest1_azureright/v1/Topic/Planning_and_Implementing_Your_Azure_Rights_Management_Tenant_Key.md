...
-   [Implementando trazer a sua própria chave (BYOK)](../Topic/Planning_and_Implementing_Your_Azure_Rights_Management_Tenant_Key.md#BKMK_ImplementBYOK)
...
-   [Gerar e transferir sua chave de locatário pessoalmente](../Topic/Planning_and_Implementing_Your_Azure_Rights_Management_Tenant_Key.md#BKMK_BYOK_InPerson)

> [!IMPORTANT]
> Se você já tiver começado a usar o [!INCLUDE[aad_rightsmanagement_1](../Token/aad_rightsmanagement_1_md.md)] (o serviço está ativado) e tiver usuários que executam o Office 2010, entre em contato com o Microsoft Customer Support Services (CSS) antes de executar estes procedimentos. Dependendo do cenário e dos requisitos, talvez seja possível usar o BYOK, mas com algumas limitações ou passos adicionais.
> 
...
|-------------|--------------------|
|Uma assinatura que suporta o Azure RMS|Para obter mais informações sobre as assinaturas disponíveis, consulte a seção [Assinaturas de nuvem que suportam o Azure RMS](../Topic/Requirements_for_Azure_Rights_Management.md#BKMK_SupportedSubscriptions) no tópico [Requisitos para o Azure Rights Management](../Topic/Requirements_for_Azure_Rights_Management.md).|
|Você não pode usar o RMS para indivíduos ou Exchange Online. Ou, se você usar o Exchange Online, entenda e aceite as limitações do uso do BYOK com essa configuração.|Para obter mais informações sobre as atuais restrições e limitações do BYOK, consulte a seção [Preços e restrições do BYOK](../Topic/Planning_and_Implementing_Your_Azure_Rights_Management_Tenant_Key.md#BKMK_Pricing) neste tópico. **Important:** Atualmente, o BYOK não é compatível com o Exchange Online.|
|HSM da Thales, smartcards e software de suporte<br /><br />Se estiver migrando do AD RMS para o Azure RMS usando a chave de software para a chave de hardware, você deve ter uma versão mínima de 11.62 para os drivers do Thales.|É necessário ter acesso a um Módulo de Segurança de Hardware da Thales e um conhecimento operacional básico dos HSMs da Thales. Consulte [Thales Hardware Security Module](http://www.thales-esecurity.com/msrms/buy) para obter a lista de modelos compatíveis ou para comprar um HSM, se você não tiver um.|
|Se deseja transferir sua chave de locatário via Internet, em vez de estar fisicamente presente em Redmond, EUA:<br /><br />1.  Uma estação de trabalho x64 offline com um sistema operacional Windows mínimo do Windows 7 e software da Thales nShield que seja, pelo menos, da versão 11,62.<br />    Se essa estação de trabalho executa o Windows 7, você deve [instalar o Microsoft .NET Framework 4.5](http://go.microsoft.com/fwlink/?LinkId=225702).<br />2.  Uma estação de trabalho que esteja conectada à Internet e que tenha um sistema operacional Windows mínimo do Windows 7.<br />3.  Uma unidade USB ou outro dispositivo de armazenamento portátil que tenha, pelo menos, 16 MB de espaço livre.|Esses pré-requisitos não são necessários se você viajar para Redmond e transferir sua chave de locatário pessoalmente.<br /><br />Por razões de segurança, recomendamos que a primeira estação de trabalho não esteja conectada a uma rede. No entanto, isto não é programaticamente aplicado. **Note:** Nas instruções a seguir, essa estação de trabalho é mencionada como a estação de trabalho desconectada.<br />Além disso, se a chave de locatário for para uma rede de produção, recomendamos o uso de uma segunda estação de trabalho separada para baixar o conjunto de ferramentas e fazer o upload da chave de locatário. Mas para fins de teste, é possível usar a mesma estação de trabalho como a primeira. **Note:** Nas instruções a seguir, essa segunda estação de trabalho é mencionada como a estação de trabalho conectada à Internet.|
|Opcional: Assinatura do Azure|Se quiser registrar o uso de sua chave de locatário (e uso do Rights Management), será necessário ter uma assinatura do Azure e armazenamento suficiente no Azure para armazenar seus logs.|
...
-   [Etapa 1: Crie um mundo de segurança](../Topic/Planning_and_Implementing_Your_Azure_Rights_Management_Tenant_Key.md#BKMK_InternetGenerate1)
...
-   [Etapa 2: Crie um mundo de segurança](../Topic/Planning_and_Implementing_Your_Azure_Rights_Management_Tenant_Key.md#BKMK_GenerateYourKey2)
